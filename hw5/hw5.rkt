  ...
  (cond [(var? e) 
        ...
        [(mlet? e)
         (let ([v1 (mlet-var e)]
           ...
           ;        (error "MUPL invalid variable name")))]
         ...
         (let ([clo (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? clo)
               (let* ([f (closure-fun clo)]
                     [new-env (cons (cons (fun-formal f) arg) (closure-env clo))])
                     (if (fun-nameopt f)
                         (eval-under-env (fun-body f) (cons (cons (fun-nameopt f) clo) new-env))
       ...
       (fun #f "mupl-lst"
            (ifaunit (var "mupl-lst")
                     ...
                     (apair (call (var "mupl-fun") (fst (var "mupl-lst")))
                            (call (call (var "mupl-map") (var "mupl-fun"))
                                  (snd (var "mupl-lst"))))))))
